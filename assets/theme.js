 ...
 * ------------------------------------------------------------------------------
 *
 * A selection of functions to help with the setting, reading cookes
 *)
 */

 theme.Cookies = (function() {
...
theme.Accordian = (function() {
  function Accordian(containerId) {
    var selectors = {
      container: '#' + containerId,
      accordianEl: '#' + containerId + ' .accordian__el'
    }
  ...
  Accordian.prototype.init = function() {
    var accordian = this.selectors.accordianEl

    $(accordian).on('click', function() {
      var selector_id = $(this).data('accordian-id')
      $(accordian + ' .accordian__el-child[data-accordian-id-child=' + selector_id + ']').toggleClass('show')
    })
...
theme.Header = (function() {
  var selectors = {
    ...
    siteAnnouncementBar: '.announcement-bar',
    siteAccouncementBarClose: '.announcement-bar__close'
  };
  ...
  function init() {
    ...
    $(selectors.siteAccouncementBarClose).on('click', function() {
      hideAnnouncementBar()
    })
  ...
  function initAccouncementBar() {
    var announcement_inactive = theme.Cookies.getCookie("announcement_bar")
    if(announcement_inactive) {
      $(selectors.siteAnnouncementBar).css('display', 'none')
    } else {
      $(selectors.siteAnnouncementBar).css('display', 'block')
    }
  }

  function hideAnnouncementBar() {
    theme.Cookies.setCookies('announcement_bar', 'true', 30)
    $(selectors.siteAnnouncementBar).animate({
      height: "toggle"
    }, 500, function() {
      $(selectors.siteAnnouncementBar).css('display', 'none')
    });
...
theme.MobileNav = (function() {
  ...
  function init() {
    ...
    // Position menu appropriate distance left
    var translateHeaderHeight = cache.$siteHeader.outerHeight();

    cache.$mobileNavContainer.css({
      transform: 'translate(-100%,' + translateHeaderHeight + 'px)'
    });
  ...
  function openMobileNav() {
    ...
    var minMenuHeight = $(window).outerHeight() - cache.$sectionHeader.outerHeight() 

    cache.$mobileNavContainer.css({
      'transform': 'translate(0px,' + translateHeaderHeight + 'px)',
      'min-height': minMenuHeight + 'px'
    });

    var minHeight = cache.$mobileNavContainer.outerHeight()
    ...
    // Lock background
    var fixedBackgroundHeight = cache.$mobileNavContainer.outerHeight() 

    cache.$pageContainer.css({
      height: fixedBackgroundHeight + 'px'
    });

    cache.$pageContainer.addClass('page-container__overlay');
  ...
  function closeMobileNav() {
    ...
    cache.$pageContainer.removeClass('page-container__overlay');

    cache.$pageContainer.off('click')
  ...
  function toggleSubNav(evt) {
    ...
    var menuTarget = $('.mobile-nav__dropdown[data-parent="' + target + '"]')
  ...
  function goToSubnav(target) {
    ...
    $targetMenu.one(
      'animationend.subnavToggle webkitAnimationEnd.subnavToggle oAnimationEnd.subnavToggle MSAnimationEnd.subnavToggle',
      function() {
        slate.a11y.trapFocus({
          $container: $container,
...
theme.Search = (function() {
  ...
  function init() {
    ...
    $(selectors.searchHeaderInput).add(selectors.searchHeaderSubmit)
...
theme.Slideshow = (function() {
  ...
  function slideshow(el, sectionId) {
    ...
    this.$mediumAndUp = settings.mediumAndUp;
    ...
    this.$slideshow.slick('slickUnfilter');
    var documentWidth = $(document).width()
    if (documentWidth < this.$mediumAndUp) {
      this.$slideshow.slick('slickFilter', '.slideshow__slide--mobile')
    } else {
      this.$slideshow.slick('slickFilter', '.slideshow__slide--desktop')
    }

    $(window).resize(filterMobile.bind(this));
  }
  ...
  slideshow.prototype.setSlideshowHeight = function() {
    ...
    var minAspectRatioMobile = this.$slideshow.data('min-aspect-ratio_mobile')
    var documentWidth = $(document).width()
    if (documentWidth > this.$mediumAndUp && minAspectRatio) {
      this.$slideshow.height(documentWidth / minAspectRatio);
    } else if (documentWidth < this.$mediumAndUp && minAspectRatioMobile) {
      this.$slideshow.height(documentWidth / minAspectRatioMobile);
    } else {
...
theme.Collection = (function() {
  var selectors = {
    collectionItem: '.grid__item--collection-template',
    productFocus: '.product-focus',
    productFocusShow: 'product-focus__show',
    firstBlock: 'first_block',
    closeButton: '.row-container__close'
  } 

  function Collection(container) { 
    this.$container = $(container)
    this.$productFocusShowClass = $('.' + selectors.productFocusShow)
    this.$productFocusShow = selectors.productFocusShow
    this.$firstBlock = selectors.firstBlock
    this.$productFocus = selectors.productFocus
    var parent = this

    $(selectors.collectionItem).on('click', function(event) {


      if($(event.target).hasClass('row-container__close-button')) {
        parent.$container.find('.' + parent.$productFocusShow).removeClass(parent.$productFocusShow)
        parent.$container.find('.row-container').removeClass('row-container__shadow')
        parent.$container.find('.row-container').css('min-height', 'inherit');
      } else {
        var clicked_collection_el = $(this)
        var clicked_collection_el_check = clicked_collection_el.find('.product-focus__show')
          
        if(clicked_collection_el_check.length == 0) {
          parent.$container.find('.' + parent.$productFocusShow).removeClass(parent.$productFocusShow)
          parent.$container.find('.row-container').removeClass('row-container__shadow')
          parent._switchBlocks(clicked_collection_el)
          parent._animateShadow(clicked_collection_el)
          setTimeout(parent._animateFocus.bind(null, clicked_collection_el), 100);
    ...
    Collection.prototype = _.assignIn({}, Collection.prototype, {
      ...
      _rowSizeAdjust: function() {
        $('.product-focus__show').css('height', 0)
        var rowMinHeight = $('.product-focus__show').prop('scrollHeight') + 40
        $('.row-container__shadow').css('min-height', rowMinHeight + 'px')
        var productFocusShowHeight = $('.row-container__shadow').prop('scrollHeight') - 40
        $('.product-focus__show').css('height', productFocusShowHeight + 'px')
      },

      _switchBlocks: function($el) {
        var row = $el.data('row')
        var deviceRowContainer = '.row-container__desktop'
        if($(window).width() < '750') {deviceRowContainer = '.row-container__mobile'}
        $(deviceRowContainer + ' .first_block[data-row=' + row + ']').before($el)
        $(deviceRowContainer + ' .first_block[data-row=' + row + ']').removeClass('first_block')
        $el.addClass('first_block')
      },

      _animateFocus: function($el) {
        $el.find('.product-focus').addClass('product-focus__show')
      },

      _animateShadow: function($el) {
        var deviceRowContainer = '.row-container__desktop'
        if($(window).width() < '750') {deviceRowContainer = '.row-container__mobile'}
        $el.closest(deviceRowContainer).addClass('row-container__shadow')
      }
    })
...
theme.Multifilters = (function() {
  function MultiFilter(container) {
    var $container = (this.$container = $(container));

    this.$multiFilterButton = $('#multifilter')
    this.$multiFilter = $('#multifilter__options')

    this.$multiFilterButton.on('click', this._onFilterChange.bind(this))
  }

  MultiFilter.prototype = _.assignIn({}, MultiFilter.prototype, {
    _onFilterChange: function() {
      // Get filter value
      var filters = '';


      $.each($(".multifilter-select"), function() {
        var added_filter = $(this).val()

        if(added_filter) {
          var concat_added_filter = added_filter + '+'
          filters += concat_added_filter
        }
      });

      filters = filters.substring(0, filters.length - 1);

      var urlStripped = window.location.href.split("multifilter")[0]
      var urlStripped = urlStripped.split("?")[0]
      urlStripped = urlStripped.substring(urlStripped.length-1) == "/" ? urlStripped : urlStripped + '/';

      var urlParameter = window.location.href.split("?")[1]
      urlParameter = typeof urlParameter !== 'undefined' ? '?' + urlParameter : '';

      document.location.href = urlStripped + filters + urlParameter;
    }
  })
...
theme.FooterSection = (function() {
  function Footer() {
    theme.Accordian = new theme.Accordian('footer-accordian');
  }
...
theme.Product = (function() {
  function Product(container) {
    ...
    this.selectors = {
      ...
      variantSelector: '.single-option-selector'
    };
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _initBuyButton: function() {
      var variantSelectors = $(this.variantSelector);
      var cartEnabled = true;
      var self = this;
      variantSelectors.on('change', function() {
        self._checkVariants()
      });
    },

    _checkVariants: function() {
      var variantSelectors = $(this.variantSelector)
      var cartEnabled = true;

      variantSelectors.each(function() {
        var selectedOption = $(this).find('option:selected').attr('disabled')
    ...
    _initImageSlider: function() {
      var options = {
        ...
        arrows: false,
        autoplaySpeed: 2000,
        // appendDots: $('.slideshow__arrows')
      };

      var productSlider = $(this.selectors.productSinglePhotos)

      productSlider.slick(options);

      // Accessibility concerns not yet fixed in Slick Slider
      $(this.selectors.productSinglePhotos, this.$container)
        .find('.slick-list')
...
theme.Instafeed = (function() {
  ...
  var mobileOptions = _.assignIn({}, defaults,{
    slidesToScroll: 1,
    slidesToShow: 4,
    arrows: false,
  })

  function instafeed(container) {
    var $container = (this.$container = $(container));
    var sectionId = $container.attr('data-section-id');
    var $feedSlider = $('.feed-slider--' + sectionId)
    var mobileSlider = false;
    var feedSliderInitiliased = false;

    $('#insta-feed').bind('DOMNodeInserted', function() {
        if($(this).children().length > 5 && feedSliderInitiliased == false) {
          feedSliderInitiliased = true
          if($(window).width() >= config.mediumAndUp) {
            initSlider($feedSlider, defaults)
          } else {
            initSlider($feedSlider, mobileOptions)
            mobileSlider = true
          }
        }
    });
    ...
    $(window).resize(function() {
      if(feedSliderInitiliased) {
        if(!mobileSlider && $(window).width() < config.small) {
          mobileSlider = true;
          $feedSlider.slick('slickSetOption', {slidesToShow: 4, slidesToScroll: 1});
        } else if(mobileSlider && $(window).width() >= config.mediumAndUp) {
          mobileSlider = false
          $feedSlider.slick('slickSetOption', {slidesToShow: 6, slidesToScroll: 6});
        }
      }
    })
...
theme.productSlideshows = {};

theme.ProductSlideshowSection = (function() {
  var defaults = {
    accessibility: true,
    arrows: false,
    dots: true,
  ...
  function ProductSlideshowSection(container) {
    var $container = (this.$container = $(container));
    var sectionId = $container.attr('data-section-id');
    var wrapper = (this.wrapper = '.product-slideshow-wrapper');
    var slider = (this.slider = '#ProductSlideshowWrapper-' + sectionId);
    var $slider = $(slider);
    var mobileSlider = false;
    ...
    $(window).resize(function() {
      if(!mobileSlider && $(window).width() <= config.mediumAndUp) {
        mobileSlider = true;
        $slider.slick('slickSetOption', 'slidesToShow', 2);
      } else if(mobileSlider && $(window).width() > config.mediumAndUp) {
        mobileSlider = false
        $slider.slick('slickSetOption', 'slidesToShow', 4);
      }
    })
  ...
  return ProductSlideshowSection;
})();
...
theme.ProductRecommendations = (function() {
  var selectors = {
    ...
    recommendationsInner: '.product-recommendations__slider'
  };
  ...
  ProductRecommendations.prototype = _.assignIn(
    ...
    ProductRecommendations.prototype,
    {
      ...
      _initImageSlider: function() {
        ...
        var recommendationsSlider = $(selectors.recommendationsInner)

        recommendationsSlider.slick(options);

        $(window).on('load resize', function () {
          var recommendationsSlider = $(selectors.recommendationsInner)

          if ($(window).width() > 750 && recommendationsSlider.slick('slickGetOption','slidesToShow') == 2) {
            recommendationsSlider.slick('slickSetOption', 'slidesToShow', 4, true).slick('slickSetOption', 'slidesToScroll', 4, true);
          } else if ($(window).width() < 750 && recommendationsSlider.slick('slickGetOption','slidesToShow') == 4) {
            recommendationsSlider.slick('slickSetOption', 'slidesToShow', 2, true).slick('slickSetOption', 'slidesToScroll', 2, true);
          }
        })

        // Accessibility concerns not yet fixed in Slick Slider
        $(selectors.recommendationsInner, this.$container)
          .find('.slick-list')
          .removeAttr('aria-live');
        $(selectors.recommendationsInner, this.$container)
          .find('.slick-disabled')
...
$(document).ready(function() {
  ...
  sections.register('instafeed-section', theme.Instafeed);
  sections.register('product', theme.Product);
  sections.register('collection-template', theme.Filters);
  sections.register('collection-template', theme.Collection);
  sections.register('collection-template', theme.Multifilters);
  sections.register('product-template', theme.Product);
  sections.register('header-section', theme.HeaderSection);
  sections.register('map', theme.Maps);
  sections.register('product-slideshow-section', theme.ProductSlideshowSection);
